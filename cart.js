...
function loadCart() {
  ...
  if (!userData) {
    // If no user is logged in, hide cart items and order button
    cartContainer.innerHTML = '<p>You need to log in to see your cart and place an order.</p>';
  ...
  if (cart.length === 0) {
    ...
    // Show cart items for the logged-in user only
...
document.querySelector('.order-button').addEventListener('click', function() {
  ...
  let orderDetails = cart.map((item, index) => `${index + 1}. ${item.name} - ${item.price}`).join('\n');
  const orderMessage = `
  New Order from Cart:
${orderDetails}
